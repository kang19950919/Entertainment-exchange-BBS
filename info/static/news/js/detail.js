...
$(function(){
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp.errno == '0') {
                    ...
                    comment_html +='<a href="javascript:;" class="comment_up fr" data-commentid="'+ comment.id + '" data-newsid="'+ comment.news_id + 'data-like-count='+ comment.like_count +'">赞</a>'
                    ...
                    // 清空输入框内容
                    $(".comment_input").val("")
                    updateCommentCount(resp.comments)
                }else {
                    alert(resp.errmsg)
                }
            }
    ...
    $('.comment_list_con').delegate('a,input','click',function(){
            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp.errno == "0") {
                        ...
                        // 更新点赞数据
                        $this.attr('data-like-count', like_count)
                        if (like_count == 0) {
                            $this.html("赞")
                        }else {
        ...
        {
            // alert(sHandler.indexOf('reply_sub'))
...
function updateCommentCount(resp) {
    // var count = $(".comment_list").length
    // $(".comment_count").html(count +"条评论")
    $(".comment_count").html(resp + "条评论")
}